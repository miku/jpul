<?php if ($models): ?>
	...
	<p class="alignright">
		...
		<?php if ($params["page"] > 1): ?>
			...
			?>
			<a href="<?php echo $this->createUrl('admin/index', $params_for_previous); ?>">&lt; Vorherige</a>
			
			<!-- shortcut -->
			<!-- <?php $params_for_first = $params; $params_for_first['page'] = 1; ?>
			<a href="<?php echo $this->createUrl('admin/index', $params_for_first); ?>">1</a> -->
		...
		<?php if ($number_of_pages == 1): ?>
			<!-- CURRENT PAGE -->
			<a class="current-page" href="<?php echo $this->createUrl('admin/index', $params) ?>"><?php echo $params["page"] ?></a>
			
		<?php elseif ($number_of_pages <= 7): ?>		
			
			<?php for ($i = 1; $i <= $number_of_pages; $i++): ?>
				<?php $params_for_page = $params; $params_for_page["page"] = $i ?>
				<a class="<?php if ($i == $current_page) { echo 'current-page'; } ?>" href="<?php echo $this->createUrl('admin/index', $params_for_page) ?>">
		...
		<?php if ($current_end < $total): ?>
			...
			?>
			<a href="<?php echo $this->createUrl('admin/index', $params_for_next) ?>">Nächste &gt;</a>
		<?php else: ?>
